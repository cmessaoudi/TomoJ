...
if (name.endsWith(".dat") && 1 == buf[1] && 0 == buf[2]) { // 'new format' only
    return tryPlugIn("Open_DAT_EMMENU", path);
        ...
        //mrc from Imod for extension st, ali or rec
        if (name.endsWith(".mrc")||name.endsWith(".st")||name.endsWith(".ali")||name.endsWith(".rec")) {
            imp = (ImagePlus) IJ.runPlugIn("fr.curie.InputOutput.MRC_Reader", path);
            if (imp == null) {
                width = PLUGIN_NOT_FOUND;
            }
            if (imp != null && imp.getWidth() == 0) {
        ...
        //mrc.bz2
        if (name.endsWith(".mrc.bz2")) {
            imp = (ImagePlus) IJ.runPlugIn("fr.curie.InputOutput.MRC_BZ2_Reader", path);
            if (imp == null) {
                width = PLUGIN_NOT_FOUND;
            }
            if (imp != null && imp.getWidth() == 0) {
        ...
        if (name.toLowerCase().endsWith(".spi") || name.toLowerCase().endsWith(".xmp") || name.toLowerCase().endsWith(".vol")) {
            imp = (ImagePlus) IJ.runPlugIn("fr.curie.InputOutput.Spider_Reader", path);
            if (imp == null) {
                width = PLUGIN_NOT_FOUND;
            }
            if (imp != null && imp.getWidth() == 0) {
        ...
        if (name.toLowerCase().endsWith(".sel")) {
            imp = (ImagePlus) IJ.runPlugIn("fr.curie.InputOutput.Sel_Reader", path);
            if (imp == null) {
                width = PLUGIN_NOT_FOUND;
            }
            if (imp != null && imp.getWidth() == 0) {
        ...
        if (name.endsWith(".em")) {
            imp = (ImagePlus) IJ.runPlugIn("fr.curie.InputOutput.EM_Reader", path);
            if (imp == null) {
                width = PLUGIN_NOT_FOUND;
            }
            if (imp != null && imp.getWidth() == 0) {
