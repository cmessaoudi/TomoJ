...
/**
 * Class for dual-axes landmarks-based alignment.
 *
 * @author Antoine Cossa
 * Date: 28/03/2019
 */
public class AlignmentLandmarkDualImproved implements Alignment {
    public BufferedWriter out;
    private int nLandmarks;
    private int nImages;
    private ArrayList<ProjectionMatrix> currentAi;
    private ArrayList<ProjectionMatrix> previousAi;
    private ArrayList<DoubleMatrix2D> current_di;
    private ArrayList<DoubleMatrix2D> previous_di;
    private ArrayList<DoubleMatrix2D> current_rj;
    private ArrayList<DoubleMatrix2D> previous_rj;
    private ArrayList<DoubleMatrix2D> Wj;
    private ArrayList<ArrayList<DoubleMatrix2D>> eij;
    private ArrayList<DoubleMatrix1D> landmarkErrorsj;
    //double psiMax = 0;
    ...
    public AlignmentLandmarkDualImproved(String path, SuperTomoJPoints stp) {
        this.stp = stp;
        current_di = new ArrayList<DoubleMatrix2D>();
        currentAi = new ArrayList<ProjectionMatrix>();

        nImages = stp.getTotalNumberOfImages();
        nLandmarks = stp.getNumberOfPoints();

        loadFromFile(path);
    }

    /**
     * Not sure if this properly works !
     */
    public AlignmentLandmarkDualImproved(String dir, String[] filename, SuperTomoJPoints stp, double binning) throws IOException {
        this.stp = stp;
        current_di = new ArrayList<DoubleMatrix2D>();
        currentAi = new ArrayList<ProjectionMatrix>();
        nImages = stp.getTotalNumberOfImages();
        nLandmarks = stp.getNumberOfPoints();
//        if (!loadFromFile2(dir + filename, binning)) return; // TODO
    }

    /**
     * Dual-tilt alignment constructor.
     *
     * @param stp     SuperTomoJPoints
     * @param options Alignment options
     * @author Antoine Cossa
     * @see SuperTomoJPoints
     */
    public AlignmentLandmarkDualImproved(SuperTomoJPoints stp, AlignmentLandmarksOptions options) {
        this.stp = stp;
        ...
        for (int i = 0; i < nImages; i++) {
            ...
            if (i < stp.getNumberOfImages(0)) { // i < nImagesi of TP1
                // TP1
                currentAi.add(new ProjectionMatrix(stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(i)));
                previousAi.add(new ProjectionMatrix(stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(i)));
            } else {
                // TP2
                currentAi.add(new ProjectionMatrix(stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(i - stp.getNumberOfImages(0))));
                previousAi.add(new ProjectionMatrix(stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(i - stp.getNumberOfImages(0))));
            }
        }
        //System.out.println("create Alignment Landmark : Ai("+i+") : theta:"+currentAi.get(i).theta);
    ...
    /**
     * clear current parameters to default ones for dual tilt alignment
     */
    public void clear() {
        AlignmentLandmarkDualImproved tmp = new AlignmentLandmarkDualImproved(stp, options);
        this.copyFrom(tmp);
    }
     ...
     */
    public void copyFrom(AlignmentLandmarkDualImproved other) {
        this.stp = other.stp;
...
////    }
    ...
    /**
     * Set the tilt axis parameters for both series
     *
     * @param alpha1 angle to put tilt axis vertical for the 1st series (Y axis)
     * @param alpha2 angle to put tilt axis vertical for the 2nd series (Y axis)
     * @param beta1  perpendicularity of beam for the 1st series
     * @param beta2  perpendicularity of beam for the 2nd series
     */
    public void setUaxis(double alpha1, double alpha2, double beta1, double beta2) {
        //verbose=true;
        if (verbose) System.out.println("set uaxis(" + alpha1 + ", " + beta1 + "; " + alpha2 + ", " + beta2 + ")");
        for (int i = 0; i < nImages; i++) {
            if (i < stp.getnImagesi(0)) {
                currentAi.get(i).setUaxis(alpha1, beta1);
            } else {
                currentAi.get(i).setUaxis(alpha2, beta2);
            }
...
//    }
     ...
     */
    private void copyPreviousToCurrent() {
        for (int i = 0; i < nImages; i++) {
    ...
    public DoubleMatrix2D updateRj(int j) {
        ...
        for (int i = 0; i < nImages; i++) {
            if (stp.getPoint(j, i) != null && Wj.get(j) != null) {
    ...
    private DoubleMatrix2D firstRj(int j) {
        DoubleMatrix2D currentRj = current_rj.get(j);
        ...
        if (stp.getnImagesi(0) != -1 && stp.getnImagesi(1) != -1) {
            for (int i = 0; i < nImages; i++) {
//                if (i < nImages1) {
                if (stp.getPoint(j, i) != null && Wj.get(j) != null) {
                    ...
                    Point2D pij = stp.getCenteredPoint(j, i);
...
//                    if (j >= nLandmarks1 - 1) {
//                        if (j >= nLandmarks1) {
//                            if (stp.getPoint(j - nLandmarks1, i - nImages1) != null && Wj.get(j) != null) {
//                                DoubleMatrix2D tmp = currentAi.get(i).getAi().viewDice().zMult(currentAi.get(i).getAi(),null);
//                                sumAitAi.assign(tmp, DoubleFunctions.plus);
//                                Point2D pij = stp.getCenteredPoint(j - nLandmarks1, i - nImages1);
//                                DoubleMatrix2D Pij = DoubleFactory2D.dense.make(2,1);
//                                Pij.setQuick(0,0,pij.getX());
//                                Pij.setQuick(1,0,pij.getY());
//                                //Pij.setQuick(0,0,pij.getX()-current_di.get(i).getQuick(0,0));
...
//                    }
    ...
    public void firstdis() {
        ...
        for (int j = 0; j < stp.getSize(); j++) {
            for (int i = 0; i < nImages; i++) {
                ...
                AffineTransform t = stp.getTomoJPoints(j).getTiltSeries().getAlignment().getTransform(i);
    ...
    private void compute_eij(int i, int j) {
        Point2D pij;
        pij = stp.getCenteredPoint(j, i);
    ...
    private void computeWjs(final double lambda) {
        for (int i = 0; i < nImages; i++) {
            currentAi.get(i).clearAi();
            previousAi.get(i).clearAi();
        }
    ...
    private void computeDeformation(boolean doMagnification, boolean doShrinkage, boolean doScalingX, boolean doShearing, boolean smooth) {
...
//        error+=computeErrorParameters();
     ...
     *
     * @return array with errors [0] is isolated residual (worst error on chains)
     * [1] is average residual (average distance error)
     * [2] Mahalanobis of average error (not distance)
     * [3] average error on images
     */
...
//    }
    ...
    private DoubleMatrix2D computeWj(int j, double lambda) {
    ...
    private void update_di(int i) {
        ...
        try {
            ...
            iae.printStackTrace(); // Print error
//            System.exit(1); // Kill program (to be removed in final version)
//            interrupt = true;
        }
    ...
    private void update_tis(boolean smooth) {
        for (int i = 0; i < nImages; i++) {
            currentAi.get(i).clearAi();
            previousAi.get(i).clearAi();
        }
        ...
        int width = stp.getTomoJPoints(0).getTiltSeries().getWidth();
    ...
    private void update_mis(boolean smooth) {
        for (int i = 0; i < nImages; i++) {
            currentAi.get(i).clearAi();
            previousAi.get(i).clearAi();
        }
    ...
    private void update_sis(boolean smooth) {
        for (int i = 0; i < nImages; i++) {
            currentAi.get(i).clearAi();
            previousAi.get(i).clearAi();
        }
    ...
    private void update_deltais(boolean smooth) {
        for (int i = 0; i < nImages; i++) {
            currentAi.get(i).clearAi();
            previousAi.get(i).clearAi();
        }
        ...
        double avgdeltai = 0;
        for (ProjectionMatrix tmp : currentAi) {
            avgdeltai += tmp.deltai;
        }
        avgdeltai /= currentAi.size();
        //correct values
        for (ProjectionMatrix tmp : currentAi) {
            tmp.deltai -= avgdeltai;
        }
        ...
        if (smooth) {
            ...
            for (int i = 0; i < nImages - 1; i++) {
                ...
                int count = 0;
                if (i - 2 >= 0) {
                    sumdeltai += currentAi.get(i - 2).deltai;
                    count++;
                }
                if (i - 1 >= 0) {
    ...
    private void update_deltai(int i) {
        if (interrupt) return;
    ...
    private void update_psii(int i) {
        if (interrupt) return;
...
//                System.out.println("HRthetaDirj:"+HRthetaDirj);
    ...
    private void removeLandmark(int j) {
        stp.removeSetOfPoints(j);
        current_rj.remove(j);
...
//            if (j >= offset && j < stp.getnLandmarksi(i) + offset) {
//
//            } else offset += stp.getnLandmarksi(i);
//        }
    }

    private void saveLandmarksErrors() {
        FileInfo fi = stp.getTomoJPoints(0).getTiltSeries().getOriginalFileInfo();
        if (fi == null) {
            //System.out.println("original File Info null");
            fi = stp.getTomoJPoints(0).getTiltSeries().getFileInfo();
        }
        String savedir = (fi != null && !fi.directory.equalsIgnoreCase("")) ? fi.directory : IJ.getDirectory("current");
        savedir += stp.getTomoJPoints(0).getTiltSeries().getTitle() + stp.getTomoJPoints(1).getTiltSeries().getTitle() + "_LandmarksErrors.txt";
    ...
    public void saveLandmarksErrors(String path) {
        ...
        try {
            ...
            System.out.println("IOException at \"AlignmentLandmarkDualImproved.saveLandmarksErrors(java.lang.String)\":\n" + ioe.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private DoubleMatrix1D computeThresholds(double k) {
        int n = 0;
...
//        System.out.println("before removing ouliers from 1: " + stp.getNumberOfPoints() + " landmarks");
//        //remove outliers
//        for (int j = stp.getNumberOfPoints() - 1; j >= 0; j--) {
//            DoubleMatrix1D error = landmarkErrorsj.get(j);
...
//    }
    ...
    public float[] applyTransformOnImage(TiltSeries ts, int index) {
        float[] res;
        int index2=index;
        if(ts==stp.getTomoJPoints(1).getTiltSeries()){
            index2=index+stp.getnImagesi(0);
            //System.out.println("AlignDual applyImage changing index for second tilt series");
        }
        ImageStatistics stats=ts.getImageStatistics(index);
        if(ts.getAlignMethodForReconstruction()== TiltSeries.ALIGN_NONLINEAR){
            res=produceAlignedImage(index2,ts.getFillType() == TiltSeries.FILL_NaN ? Float.NaN : 0);
            if (ts.isNormalized()) {
                res =   ts.normalizeImage(res, stats);
            }
        } else {
            AffineTransform T= (ts.getAlignMethodForReconstruction()==TiltSeries.ALIGN_AFFINE2D)?getTransform(index2):getTranslationTransform(index2);
    ...
    public float[] produceAlignedImage(int i, float fillValue) {
        ...
        ImageProcessor ori;
        if (i < stp.getnImagesi(0)) {
            ts = stp.getTomoJPoints(0).getTiltSeries();
            ori = ts.getImageStack().getProcessor(i + 1);
        } else {
            ts = stp.getTomoJPoints(1).getTiltSeries();
            ori = ts.getImageStack().getProcessor(i - stp.getnImagesi(0) + 1);
        }
...
//        DoubleMatrix2D Ralpha_inv = MatrixUtils.rotation3DMatrixZ(currentAi.get(i).rot - 90);
        DoubleMatrix2D Ralpha_inv = MatrixUtils.rotation3DMatrixZ(currentAi.get(i).rot - 90 - 45); // TEST by Antoine
    ...
    public String toString() {
        ...
        for (int i = 0; i < nImages; i++) {
            if (i < stp.getnImagesi(0)) {
                result += "Image " + i + " " + stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(i) + "\u00b0\ndi=" + toString(current_di.get(i)) + " \npsi=" + currentAi.get(i).psii + "\n" +
                        "ti=" + currentAi.get(i).ti + " \t mi=" + currentAi.get(i).mi + "\nsi=" + currentAi.get(i).si + " \t deltai=" + currentAi.get(i).deltai + "\n";
            } else {
                result += "Image " + i + " " + stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(i - stp.getnImagesi(0)) + "\u00b0\ndi=" + toString(current_di.get(i)) + " \npsi=" + currentAi.get(i).psii + "\n" +
                        "ti=" + currentAi.get(i).ti + " \t mi=" + currentAi.get(i).mi + "\nsi=" + currentAi.get(i).si + " \t deltai=" + currentAi.get(i).deltai + "\n";
            }
    ...
    private void loadFromFile(String path) {
        try {
            ...
            String line = in.readLine(); // To skip the 1st line
            int i = 0;
            while ((line = in.readLine()) != null) {
                ...
                di.setQuick(0, 0, Double.parseDouble(words[5]));
                di.setQuick(1, 0, Double.parseDouble(words[6]));
                current_di.add(di);
                ProjectionMatrix Ai;
                if (i < stp.getnImagesi(0)) {
                    Ai = new ProjectionMatrix(stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(Integer.parseInt(words[1])));
                } else {
                    Ai = new ProjectionMatrix(stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(Integer.parseInt(words[1]) - stp.getnImagesi(0)));
                }

                Ai.psii = Double.parseDouble(words[7]);
                Ai.mi = Double.parseDouble(words[8]);
                Ai.ti = Double.parseDouble(words[9]);
                Ai.si = Double.parseDouble(words[10]);
                Ai.deltai = Double.parseDouble(words[11]);
                Ai.computeAi();
                currentAi.add(Ai);
                i++;
            }
    ...
    public boolean loadFromFile2(String path, double binning) throws IOException {
        ...
        for (int i = 0; i < nImages; i++) {
            ...
            ProjectionMatrix Ai;
            if (i < stp.getnImagesi(0)) {
                Ai = new ProjectionMatrix(stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(i));
            } else {
                Ai = new ProjectionMatrix(stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(i - stp.getnImagesi(0)));
            }
    ...
    public ResultsTable getAsResultTable() {
        ...
        for (int i = 0; i < nImages; i++) {
            ...
            /*if (i < stp.getnImagesi(0)) {
                rt.addValue("tilt angle", stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(i));
            } else {
                rt.addValue("tilt angle", stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(i - stp.getnImagesi(0)));
            }*/
    ...
    public void correctRjsPositionforTomogram(double zheight) {
        for (int j = 0; j < stp.getSize(); j++) {
            DoubleMatrix2D Rmin = MatrixUtils.rotation2DMatrix(90 - currentAi.get(0).rot + currentAi.get(stp.getTomoJPoints(j).getTiltSeries().getZeroIndex()).psii);
    ...
    public double correctAxisHeight() {
        ...
        int z0N = 0;
        int zeroIndex;
        double zeroTiltAngle;
        for (int j = 0; j < nLandmarks; j++) {
            //System.out.println("j="+j);
            Point2D p;
            if (j < stp.getnLandmarksi(0)) { // j < nLandmarks1
                zeroIndex = stp.getTomoJPoints(0).getTiltSeries().getZeroIndex();
                zeroTiltAngle = stp.getTomoJPoints(0).getTiltSeries().getTiltAngle(zeroIndex);
                p = stp.getTomoJPoints(0).getCenteredPoint(j, zeroIndex);
            } else {
                zeroIndex = stp.getTomoJPoints(1).getTiltSeries().getZeroIndex();
                zeroTiltAngle = stp.getTomoJPoints(1).getTiltSeries().getTiltAngle(zeroIndex);
                p = stp.getTomoJPoints(1).getCenteredPoint(j - stp.getnLandmarksi(0), zeroIndex);
            }
            if (p != null) {
...
//        for (int i = 0; i < nImages; i++) {
//            for (int j = 0; j < nLandmarks; j++) {
//                Point2D p = stp.getCenteredPoint(j, i);
//                if (p != null) {
//                    DoubleMatrix2D reprojected = DoubleFactory2D.dense.make(2, 1);
//                    currentAi.get(i).getAi().zMult(current_rj.get(j), reprojected);
//                    reprojected.assign(current_di.get(i), DoubleFunctions.plus);
...
//    /**
//     * @param index of the corresponding TomoJPoints (starts at 0)
//     * @return
//     */
//    public ArrayList<Point2D[]> getReprojectedLandmarks(int index) {
//        int nland = stp.getnLandmarksi(index);
//        int nimg = stp.getnImagesi(index);
//        ArrayList<Point2D[]> res = new ArrayList<Point2D[]>(nland);
//        for (int j = (index == 0) ? 0 : stp.getnLandmarksi(index - 1); j < nland; j++) {
//            res.add(new Point2D.Double[nimg]);
//        }
//        for (int i = (index == 0) ? 0 : stp.getnImagesi(index - 1); i < nimg; i++) {
//            for (int j = (index == 0) ? 0 : stp.getnLandmarksi(index - 1); j < nland; j++) {
//                Point2D p = stp.getTomoJPoints(index).getCenteredPoint(j, i);
//                if (p != null) {
//                    DoubleMatrix2D reprojected = DoubleFactory2D.dense.make(2, 1);
//                    currentAi.get(i).getAi().zMult(current_rj.get(j), reprojected);
//                    reprojected.assign(current_di.get(i), DoubleFunctions.plus);
...
//                }
//            }
//        } else if (index > 0 && index < stp.getSize()) {
//            int offsetJ = stp.getnLandmarksi(index - 1);
//            int offsetI = stp.getnImagesi(index - 1);
//
//            for (int j = offsetJ; j < nland + offsetJ; j++) {
//                res.add(new Point2D.Double[nimg]);
//            }
//            for (int i = offsetI; i < nimg + offsetI; i++) {
//                for (int j = offsetJ; j < nland + offsetJ; j++) {
//                    Point2D p = stp.getTomoJPoints(index).getCenteredPoint(j - offsetJ, i - offsetI);
//                    if (p != null) {
//                        DoubleMatrix2D reprojected = DoubleFactory2D.dense.make(2, 1);
//                        currentAi.get(i).getAi().zMult(current_rj.get(j), reprojected);
//                        reprojected.assign(current_di.get(i), DoubleFunctions.plus);
            ...
            for (int i = offsetI; i < nimg + offsetI; i++) {
                for (int j = offsetJ; j < nland + offsetJ; j++) {
                    ...
                    if (p != null) {
                        ...
                        currentAi.get(i).getAi().zMult(current_rj.get(j), reprojected);
                        reprojected.assign(current_di.get(i), DoubleFunctions.plus);
                        res.get(j - offsetJ)[i - offsetI] = new Point2D.Double(reprojected.getQuick(0, 0), reprojected.getQuick(1, 0));
                    }
                }
            }
        }
...
//    public ArrayList<ArrayList<Point2D[]>> getReprojectedLandmarks() {
//        ArrayList<Point2D[]> RL1 = new ArrayList<Point2D[]>(stp.getnLandmarksi(0));
//        ArrayList<Point2D[]> RL2 = new ArrayList<Point2D[]>(stp.getnLandmarksi(1));
//
//        ArrayList<ArrayList<Point2D[]>> res = new ArrayList<>(2);
//        res.add(RL1);
//        res.add(RL2);
//
//        for (int j = 0; j < stp.getnLandmarksi(0); j++) {
//            RL1.add(new Point2D[stp.getnImagesi(0)]);
//        }
//        for (int j = 0; j < stp.getnLandmarksi(1); j++) {
//            RL2.add(new Point2D[stp.getnImagesi(1)]);
//        }
//
//
//
...
//    }
    ...
    public AffineTransform[] getTransforms() {
        AffineTransform[] results= new AffineTransform[currentAi.size()];
        for(int i =0; i<results.length;i++) results[i]=getTransform(i);
        return results;
    }
    ...
    public void resetEulerMatrices() {
        System.out.println("aligndual reset euler");
        for(ProjectionMatrix pm:currentAi){
            pm.resetEuler();
        }
    }
    ...
    public DoubleMatrix2D getEulerMatrix(int index) {
        return currentAi.get(index).getEuler();
    }
    public DoubleMatrix2D getEulerMatrix(TiltSeries ts,int index) {
        if(ts instanceof TiltSeriesStack){
            ts=((TiltSeriesStack) ts).getTiltSeries(index);
        }else{
            int indextmp=index;
            int offset=0;
            for(int t=0;t<stp.getSize();t++){
                if(indextmp >= offset && indextmp < offset+stp.getnImagesi(t)){
                    index+=offset;
                }
                offset+=stp.getnImagesi(t);
            }
        }
        if(ts!= stp.getTomoJPoints(0).getTiltSeries()){
            ...
            currentAi.get(index).setTiltAxisVertical(true);
            DoubleMatrix2D euler = currentAi.get(index).getEuler();
            euler = euler.zMult(MatrixUtils.rotation3DMatrixZ(currentAi.get(index).getRot() - currentAi.get(0).getRot()), null);
        ...
        /*if(index<ts.getImageStackSize()){
            ...
            if(! (ts instanceof TiltSeriesStack)){
                ...
                int offset=0;
                for(int t=0;t<stp.getSize();t++){
                    TiltSeries tilttmp=stp.getTomoJPoints(t).getTiltSeries();
                    if(indextmp >= offset && indextmp < offset+tilttmp.getImageStackSize()){
                        index+=offset;
                    }
                    offset+=tilttmp.getImageStackSize();
                }
            }
            currentAi.get(index).setTiltAxisVertical(true);
            DoubleMatrix2D euler = currentAi.get(index).getEuler();
        ...
        /*
            //System.out.println("alignDual euler");
            if (ts == stp.getTomoJPoints(1).getTiltSeries()) {
                index += stp.getnImagesi(0);
                currentAi.get(index).setTiltAxisVertical(true);
                DoubleMatrix2D euler = currentAi.get(index).getEuler();
                euler = euler.zMult(MatrixUtils.rotation3DMatrixZ(currentAi.get(0).getRot() - currentAi.get(index).getRot()), null);
