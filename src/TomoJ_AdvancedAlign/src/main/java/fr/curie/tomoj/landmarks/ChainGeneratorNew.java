 ...
 */
public class ChainGeneratorNew {
    TomoJPoints tp;
    ...
    public ChainGeneratorNew(TomoJPoints tp,TiltSeries ts){
        this.ts=ts;
        this.tp=tp;
    ...
    public void generateLandmarkSet2(final int seqLength, final int numberOfSamples, final int localSize, final int refinementCycles, final double corrThreshold, final boolean useMinima, final boolean useCritical, final boolean fuseLandmarks) {
        ...
        System.out.println("generate Landmark V2new using " + nbThreads + " threads" + " / " + Runtime.getRuntime().availableProcessors());
        ...
        System.out.println("use critical:"+useCritical+"\t use minima:"+useMinima);
        SeedDetector sd=new SeedDetector(ts);

        /* DEBUG!!!!
        ArrayList<LandmarksChain> sdtmp=sd.createsSeedsLocalExtrema(0,true,0.1,0.1,critical_FilterSmall,critical_FilterLarge,critical_MinimaRadius,critical_SeedNumber);
        ArrayList<LandmarksChain> cgntmp=createSeeds(0,100,true,true,0.1,0.1);
        int nbdiff=0;
        for(int d=0;d<sdtmp.size()&&d<cgntmp.size();d++){
            LandmarksChain lc1=sdtmp.get(d);
            double x1=lc1.getLandmarkchain()[0].getX();
            double y1=lc1.getLandmarkchain()[0].getY();
            LandmarksChain lc2=cgntmp.get(d);
            double x2=lc2.getLandmarkchain()[0].getX();
            double y2=lc2.getLandmarkchain()[0].getY();

            if(x1!=x2 || y1!=y2) nbdiff++;
        }*/
        ...
        if(useCritical){
            Q.addAll(sd.createsSeedsLocalExtrema(useMinima,percentageExcludedX,percentageExcludedY,critical_FilterSmall,critical_FilterLarge,critical_MinimaRadius,critical_SeedNumber));
        }else{
            Q.addAll(sd.createsSeedsGrid(gridSamples,gridSamples,0,0));
        }
        //
