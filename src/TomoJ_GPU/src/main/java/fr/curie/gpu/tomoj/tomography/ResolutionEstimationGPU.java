 ...
 */
public class ResolutionEstimationGPU extends ResolutionEstimation {
    ReconstructionThreadManagerGPU recThManSignal;

    ReconstructionThreadManagerGPU recThManNoise;
    ReconstructionThreadManagerGPU recThManVSSNR;
    ReconstructionThreadManagerGPU recManFSCeven;
    ReconstructionThreadManagerGPU recManFSCodd;
    ...
    ReconstructionThreadManagerGPU currentRecThreadManager;
    ...
    public ResolutionEstimationGPU(TiltSeries tsSignal, ReconstructionParameters parameters) {
        super(tsSignal,parameters);
    }

    public ResolutionEstimationGPU(ResolutionEstimationGPU other, TiltSeries ts){
        super(ts,other.recParameters);
        this.recThManSignal=new ReconstructionThreadManagerGPU(other.recThManSignal,ts);
    }

    public ReconstructionThreadManagerGPU getRecThManSignal() {
        return this.recThManSignal;
    }

    public void doSignalReconstruction(){
        if(recThManSignal==null) recThManSignal=new ReconstructionThreadManagerGPU(tsSignal.getWindow(),tsSignal);
        else recThManSignal.setTiltSeries(tsSignal);
        if(recSignal!=null) recThManSignal.setRec2(recSignal);
        recThManSignal.setUse(use);
        recThManSignal.setDevices(devices);
    ...
    public void generateSSNRNoiseData(){
        ...
        //tsNoise.setZeroTransform(tsSignal.getZeroTransform());
        tsNoise.setFillType(tsSignal.getFillType());
        tsNoise.updateInternalData();
        tsNoise.setTomoJPoints(tsSignal.getTomoJPoints());
        if(recThManNoise==null)recThManNoise=new ReconstructionThreadManagerGPU(recThManSignal,tsNoise);
    ...
    public void generateFSCData(){
        ...
        if(recThManSignal!=null) {
            ...
            if(recManFSCeven==null){
                recManFSCeven=new ReconstructionThreadManagerGPU(tsFSCeven.getWindow(),tsFSCeven);
    ...
    public double[][] SSNR(boolean computeVSSNR){
        ...
        if (computeVSSNR){
            ...
            recThManVSSNR = new ReconstructionThreadManagerGPU(recThManSignal,tsVSSNR);
