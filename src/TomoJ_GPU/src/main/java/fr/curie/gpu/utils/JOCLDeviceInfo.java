...
//        }
    ...
    public static String getVendorName(cl_device_id device) {
        return getString(device, CL_DEVICE_VENDOR);
    }

    public static long getType(cl_device_id device) {
        return getLong(device, CL_DEVICE_TYPE);
    }

    public static long getMaxMemoryAllocationSize(cl_device_id device) {
    ...
    public static long getGlobalMemorySize(cl_device_id device) {
        return getLong(device, CL_DEVICE_GLOBAL_MEM_SIZE);
    }

    public static long getMaxWidthImage3D(cl_device_id device) {
        return getSize(device, CL_DEVICE_IMAGE3D_MAX_WIDTH);
    }
    ...
    public static long getClockFrequency(cl_device_id device) {
        return getLong(device, CL_DEVICE_MAX_CLOCK_FREQUENCY);
    }

    public static void printInfo(cl_device_id device) {
        // CL_DEVICE_NAME

        System.out.println("--- Info for device " + getName(device) + ": ---");
        ...
        // CL_DRIVER_VERSION
        String openCLVersion = getString(device, CL_DEVICE_VERSION);
        System.out.printf("CL_DEVICE_VERSION: \t\t\t%s\n", openCLVersion);


        // CL_DEVICE_TYPE
        long deviceType = getType(device);
...
//        long queueProperties = getLong(device, CL_DEVICE_QUEUE_ON_HOST_PROPERTIES);
    ...
    public static void printPlatformInfo(cl_device_id device) {
        long[] size = new long[1];
        clGetDeviceInfo(device, CL_DEVICE_PLATFORM, 0, null, size);
        cl_platform_id[] platform = new cl_platform_id[(int) size[0]];
        clGetDeviceInfo(device, CL_DEVICE_PLATFORM, platform.length * Sizeof.cl_platform_id, Pointer.to(platform), null);

        for (int i = 0; i < platform.length; i++) {
            System.out.println("platform #" + i + " name: " + getString(platform[i], CL_PLATFORM_NAME));
            System.out.println("platform #" + i + " vendor: " + getString(platform[i], CL_PLATFORM_VENDOR));
            System.out.println("platform #" + i + " version: " + getString(platform[i], CL_PLATFORM_VERSION));
            System.out.println("platform #" + i + " profile: " + getString(platform[i], CL_PLATFORM_PROFILE));
            System.out.println("platform #" + i + " extensions: " + getString(platform[i], CL_PLATFORM_EXTENSIONS));
        }
    }
