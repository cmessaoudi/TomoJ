 ...
 */
public class BgARTVoxelProjector3D extends VoxelProjector3D {
    double k;
    boolean darkBG = true;
    boolean doMedian = true;
    double bgValue = Double.NaN;

    public BgARTVoxelProjector3D(TiltSeries ts, TomoReconstruction2 rec, FFTWeighting weightingFilter, double k, boolean darkBG, boolean doMedian) {
                ...
                for (int x = 0; x < volumeWidth; x++) {
                    ...
                    for (int j = -radius; j <= radius; j++) {
                        for (int i = -radius; i <= radius; i++) {
                            if (x + i < 0 || x + i >= volumeWidth || y + j < 0 || y + j >= volumeHeight) continue;
                            //n[count++]= tmp.getPixel(x+i,y+j,z+k);
                            n[count++] = pixs[(j + y) * volumeWidth + x + i];
                            n[count++] = (pixsmin != null) ? pixsmin[(j + y) * volumeWidth + x + i] : 0;
                            n[count++] = (pixsmax != null) ? pixsmax[(j + y) * volumeWidth + x + i] : 0;

                        }
                    }
                    Arrays.sort(n);
                    rec.putPixel(x, y, z, n[13]);
                }
            }
        }
    }
    ...
    public void startOfIteration() {
        if (doMedian) medianFilter3Dcross();
