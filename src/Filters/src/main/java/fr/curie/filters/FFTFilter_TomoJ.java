 ...
 */
public class FFTFilter_TomoJ implements PlugInFilter, Measurements {
    ...
    public int setup(String arg, ImagePlus imp) {
        ...
        if (arg != null && !arg.equals("")) {
            ...
            for (String word : words) {
                if (word.startsWith("filter_large")) {
                    filterLargeDia = Double.parseDouble(word.split("=")[1]);
                }
                if (word.startsWith("filter_small")) {
                    filterSmallDia = Double.parseDouble(word.split("=")[1]);
                }
                if (word.startsWith("suppress")) {
                    //not taken into account
                }
                if (word.startsWith("tolerance")) {
                    toleranceDia = Double.parseDouble(word.split("=")[1]);
                }
            }
        } else if (!showBandpassDialog(imp))
            return DONE;
    ...
    public void filter(ImageProcessor ip) {
        ...
        fht.transform();
        time.stop();
        //System.out.println("fht:"+time.delayString());
        ...
        time.stop();
        //System.out.println("filter:"+time.delayString());
        ...
        time.stop();
        //System.out.println("fht inverse:"+time.delayString());
    ...
    public ImageProcessor tileMirror(ImageProcessor ip, int width, int height, int x, int y) {
        ...
        if (stopProcessing) return ipout;
        //tile
        if ((i1 % 2) > 0.5)
            ip2.flipHorizontal();
        if (stopProcessing) return ipout;
